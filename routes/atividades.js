    ...
    app.get("/excluir", async(req,res)=>{
     ...
     app.get("/entregue", async(req,res)=>{
        ...
        app.get("/desfazer", async(req,res)=>{
            ...
            //capturar o id(atvididade) da barra de endereço
            var id = req.query.id
            //buscar todas as atividades que será alterada 
            var abertas = await atividades.findOne({_id:id})
            //buscar o nome na collection usuarios
            var user = await usuarios.findOne({_id:alterar.usuario})
            //abrir a view atividades2
            res.render('alterar.ejs',{nome:user.nome,id:user._id,dados:abertas,dados:alterars})
            })
